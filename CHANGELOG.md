...
- **Total-edge count is now O(1)** — `MATCH ()-[r]->() RETURN count(r)` (and `count(*)`) now plans to a new `FusedCountAllEdges` clause that reads the `petgraph` edge count directly, instead of materializing every edge through the general MATCH executor. On the BloodHound AD comparison fixture this dropped from 41.6 ms to 360 µs (≈115×). Mirrors the existing `FusedCountAll` pattern for nodes.
- **Cheaper edge-type cache build** — `get_edge_type_counts` now counts edges by `InternedKey` (u64) and only resolves to `String` once per type at the end, eliminating the per-edge `String` allocation in the hot loop. Typed-edge counts (`MATCH ()-[r:Type]->() RETURN count(r)`) and edges-by-type (`MATCH ()-[r]->() RETURN type(r), count(*)`) both benefit when the cache is cold.
- **Cheaper typed-node count** — `FusedCountTypedNode` now uses `count_nodes_matching_label`, which reads the per-type index lengths directly instead of allocating a `Vec<NodeIndex>` just to take its length.
- **Faster bulk-edge create for new connection types** — `ConnectionBatchProcessor::flush_chunk` now skips its per-source adjacency scan when the graph contains no edges of the target type yet (common during analysis post-processing, where each derived edge type is brand-new). The within-chunk dedup path still keeps duplicate `(src,dst)` pairs collapsed via insert-on-add, and the type is registered in `connection_types` after the first add so subsequent chunks within the same `execute()` call resume building the index. Hub-source-heavy Azure post-processing — where a service-principal source has thousands of pre-existing outgoing edges of unrelated types — was previously walking that whole adjacency list per chunk just to confirm "no edges of the new derived type". The same path also reuses the already-interned connection-type key per edge (via `EdgeData::new_interned`) instead of re-interning the same `String` for every edge in the chunk.
